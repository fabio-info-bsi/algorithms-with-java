...
public class LinearSortingInt {

    private static final Logger logger = LoggerFactory.getLogger(LinearSortingInt.class);
     ...
     * @param operator
     */
    public static void insertionSort(int[] array, int endIndex, IntUnaryOperator operator) {
        for (int i = 1; i < endIndex; i++) {
            int key = operator.applyAsInt(array[i]);
            int value = array[i];
            int j = i - 1;
            while (j >= 0 && operator.applyAsInt(array[j]) > key) {
                array[j + 1] = array[j];
                j = j - 1;
            }
            array[j + 1] = value;
     ...
     * @param operator
     */
    public static void selectionSort(int[] array, int endIndex, IntUnaryOperator operator) {
        for (int i = 0; i < endIndex; i++) {
            int minIndex = i;
            for (int j = i + 1; j < endIndex; j++) {
                if (operator.applyAsInt(array[j]) < operator.applyAsInt(array[minIndex])) {
                    minIndex = j;
                }
            }
            if (minIndex != i) { // if need to exchange!
     ...
     * @param operator
     */
    public static void bubbleSortNotStable(int[] array, IntUnaryOperator operator) {
        for (int i = 0; i < array.length; i++) {
            for (int j = i; j < array.length; j++) {
                if (operator.applyAsInt(array[i]) > operator.applyAsInt(array[j])) {
                    int temp = array[i];
                    array[i] = array[j];
                    array[j] = temp;
                }
     ...
     * @param operator
     */
    public static void bubbleSort(int[] array, IntUnaryOperator operator) {
        int n = array.length;
        for (int i = 0; i < n - 1; i++)
            for (int j = 0; j < n - i - 1; j++)
                if (operator.applyAsInt(array[j]) > operator.applyAsInt(array[j + 1])) {
                    int temp = array[j];
                    array[j] = array[j + 1];
                    array[j + 1] = temp;
                }
